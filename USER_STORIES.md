...
- `US-109c` **保存过滤视图**：用户可以把常用 tag / time / status / search / summary 组合命名保存成固定视图，比如“Q1”“工作”“本周复盘”，之后一键切换。按钮文案必须是“保存过滤视图”（英文 Save filter view），点击后要求用户命名；没有任何临时筛选条件时，保存按钮 disabled 并提示先设置条件，不能保存一个和“无过滤”重复的空 preset。保存视图只是 UI preset，不改 markdown、不创建新数据模型；保存的是条件集合与汇总配置，不保存当时的任务列表快照。没有保存视图时，“无过滤”菜单要提示可以先设置条件并新建视图。时间条件保存为语义化结构：`time.scheduled` / `time.deadline` / `time.completed` / `time.created`，不保存含义不明的 `date` 字段。
...
- `US-117` 筛选栏与保存视图栏要能承载长期使用：默认视图、保存视图、临时筛选三者分清。用户切换保存视图后可以继续临时改筛选；只有点“保存/更新视图”才写入设置。移动端不能把桌面完整筛选栏塞进首屏,只能保留一个“筛选”入口；这个入口打开 bottom sheet，里面仍按 US-109a 展示搜索 / 标签 / 排期 / 更多时间 / 状态等明确控件，标签选择同样使用搜索 + 自绘 checkbox 列表，不使用原生多选 select。

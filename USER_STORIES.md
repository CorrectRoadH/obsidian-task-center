...
- `US-109c` **保存过滤视图**：用户可以把常用 tag / date / status / search / summary 组合命名保存成固定视图，比如“Q1”“工作”“本周复盘”，之后一键切换。按钮文案必须是“保存过滤视图”（英文 Save filter view），点击后要求用户命名；没有任何临时筛选条件时，保存按钮 disabled 并提示先设置条件，不能保存一个和“无过滤”重复的空 preset。保存视图只是 UI preset，不改 markdown、不创建新数据模型；保存的是条件集合与汇总配置，不保存当时的任务列表快照。没有保存视图时，“无过滤”菜单要提示可以先设置条件并新建视图。
...
- `US-109e` **时间范围选择器形态**。日期控件默认展示“日期”，点开后是自绘 popover，不用原生 select，也不把自定义路径做成手输日期表单。popover 左侧/上方是快捷范围：全部日期、逾期、今天、明天、本周、下周、本月、未排期；右侧/下方是日历视图：月份前后切换、星期列按 US-112 一周起始设置排列、日期格可点击。日历视图固定表达“开始日期 → 结束日期”的范围选择：第一次点击日期只标记开始，第二次点击日期写入闭区间 `FROM..TO`；若第二次早于第一次，自动交换开始 / 结束。popover 内必须有“清空时间”动作，能一键回到无日期过滤。工具栏按钮显示紧凑摘要（如 `04-08 - 04-23`），不能用完整 ISO 范围撑破布局。快捷范围不能渲染成一串占满宽度的大主按钮；选中态只做轻量高亮，避免压过工具栏。用户看到的日期按 locale 显示，保存视图和 CLI 表达仍用稳定 token：`all / overdue / today / tomorrow / week / next-week / month / unscheduled / YYYY-MM-DD / FROM..TO`；其中 `YYYY-MM-DD` 作为 legacy / CLI 精确日期表达可读可显示，但日历 UI 默认产出范围 token。
...
- `US-109h` **状态筛选不用原生 select**。状态控件是轻量按钮 + popover，按钮默认显示“状态”，popover 选项固定为：全部状态、待办、完成、放弃；点击选项立即过滤并关闭 popover。选中态只做轻量高亮，视觉层级低于保存过滤视图按钮，不出现系统原生菜单。

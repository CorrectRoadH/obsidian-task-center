...
- `US-168` **点击 Task Card 打开源 Markdown 编辑对话框**。作为常驻 Obsidian 的人，我点一下卡片，就能在一个对话框里看到该任务所在文件的原始 markdown，并且任务所在行自动滚动到视窗中间、光标落在该行；我可以直接编辑该任务、它的子任务和上下文。这个旅程取代旧的 hover 上下文预览、双击跳源文件、右键打开源文件，不再保留三套查看/编辑入口。这个对话框必须复用 Obsidian 原生 Markdown 编辑能力（语法高亮、Tasks/Dataview 等 markdown 体验与普通编辑器一致），不能只是 `MarkdownRenderer` 的只读预览。
  ...
  - `US-168d` 旧 hover popover、卡片双击打开源文件、右键菜单打开源文件对应实现必须删除；同一能力统一由点击 source edit dialog 承担。

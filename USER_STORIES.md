...
- `US-165` 鼠标悬停卡片一会儿弹 popover，显示父任务链 + 源文件上下文几行，知道这条任务在哪个项目下。**这是辅助路径，不是编辑主路径**；用户不应该依赖 hover 才能编辑子任务或查看完整上下文。
...
- `US-168` **点击 Task Card 打开源 Markdown 编辑对话框**。作为常驻 Obsidian 的人，我点一下卡片，就能在一个对话框里看到该任务所在文件的原始 markdown，并且任务所在行自动滚动到视窗中间、光标落在该行；我可以直接编辑该任务、它的子任务和上下文，而不用双击跳到源文件、也不用等 hover popover。这个对话框必须复用 Obsidian 原生 Markdown 编辑能力（语法高亮、Tasks/Dataview 等 markdown 体验与普通编辑器一致），不能只是 `MarkdownRenderer` 的只读预览。
  - `US-168a` 点击普通卡片 / Today 卡片 / 搜索过滤后的卡片都走同一入口，打开对话框并定位到 `task.path:task.line`。
  - `US-168b` 对话框内编辑 markdown 后关闭，vault 文件按 Obsidian 原生编辑器保存语义落盘；看板随后刷新，卡片标题、子任务、日期、tag 与状态反映最新 markdown。
  - `US-168c` 对话框打开时 hover popover 必须关闭，且卡片单击不再只是选中；需要选中态时只能作为视觉反馈，不能阻断打开编辑对话框。
  - `US-168d` 右键菜单的"打开源文件"保留为跳到普通 Obsidian leaf 的高级入口；双击不再是必要入口，可以保留为同一动作但不能成为唯一编辑路径。
...
用户每天打开插件最常见的问题是"今天先做什么"——需要在逾期、今日、收件箱三堆信息里快速找到行动项。现有"周视图"不直接回答这个问题，展示量大、操作链路长。US-720 增加一个**今日执行视图**，第一屏聚合三组关键任务并提供最核心的快捷操作。

- `US-720a` **今日视图入口**。看板有一个"今日"入口（`data-tab="today"` 的 tab 按钮或等价入口）。点击后出现今日容器（`data-view="today"`）。
- `US-720b` **三组任务列表**。今日容器内有三个分组：`data-today-group="overdue"`（逾期）、`data-today-group="today"`（今日安排）、`data-today-group="unscheduled-rec"`（未排期推荐，每组最多展示 3 条）。任务卡包含标题、来源路径提示和日期信息。
...
用户每天早上需要把"无截止日期的待办"变成"今天要做哪几件事"的决策。现有的"未排期"分组只是任务池，没有帮助用户做筛选和排程的工作流。US-721 在看板里加一个轻量的**今日计划模式**，让用户快速圈定今日任务并一键排到具体日期。

- `US-721a` **今日计划入口**。看板（或未排期分组）内有明确的"今日计划"入口，点击后进入计划模式，界面上出现计划面板（`data-view="plan-today"` 区域）。
- `US-721b` **展示可选的未排期任务**。计划模式内列出所有未排期任务（`data-plan-candidate` 元素），包含任务标题和预估时长（如果有 `⏱️` 标记）。
- `US-721c` **预估总时长与超载提醒**。当用户选中（勾选）若干任务后，界面展示已选任务的预估总时长（`data-plan-total-est`）；当总时长超过每日容量（默认 8h）时，展示超载提示（`data-plan-overload`）。
- `US-721d` **一键排程动作**。每条未排期任务有快捷按钮（`data-plan-action="schedule-today"` / `"schedule-tomorrow"` / `"schedule-week"`），点击后直接向任务写入对应 `⏳` 日期，任务离开未排期池。动作结果在任务文件里可见（e2e 读文件验证）。

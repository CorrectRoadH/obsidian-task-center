...
- `US-101` 一眼看到**本周 7 天**谁塞满了,今日高亮。切到上一周、下一周。桌面 / 大屏 week 是 7 个日期列；移动端 week 是 7 个日期 row（见 US-503）。两者只是布局不同,**日期归属、任务计数、估时合计、拖拽改期语义必须完全一致**。周视图主区域不能塌得太矮：桌面与移动端的 week 主体最小高度至少占当前 Task Center 可视高度的一半,否则 Leo 认为视觉上不成立。
...
- `US-108` 可以通过`[字段名:: 值]`比如`[estimate:: 90m]`来扩展meta。还可以给任务加上`#内容`来标记内容。应用不对于这块有特别的限制，只是一种`约定大于配置`，用户可以自定义，后续的用户场景的四象限、复盘估时都是基于这种可以自定义的配置的一种实现。**应用层不允许硬编码任何 tag / 字段名字面**——所有过滤、快捷键、菜单项都从用户配置或 markdown 现状派生。`#1象限`、`[estimate::]` 等只是文档示例里的字面量，不是应用知识。
- `US-109` **筛选要解决的问题**：任务太多时，用户要把全库任务临时缩小成“现在能处理 / 今天要看 / 本周复盘”的集合，而不是面对一排含义不明的控件。筛选只改变当前可见集合与汇总，不改 markdown、不创建任务、不改变任务状态。
  - 搜索 = 按任务标题关键字缩小集合。
  - tag = 普通用户数据；筛选栏用当前视图候选任务里实际存在的 `#tag` 做多选 select，可同时选择多个 tag。
  - 日期 = 单选日期条件控件，不是裸 `YYYY-MM-DD` 输入框；按有效排期日期筛选，常用预设放一级，自定义日期 / 范围再进入 date picker（同 US-207）。
  - 状态 = 在待办 / 已完成 / 已放弃 / 全部之间切换；默认是待办。
  - 汇总 = 对当前筛选后的集合计算任务数、估时、实际耗时、字段 sum / ratio 等复盘值。
- `US-109a` **筛选栏不是一个“筛选”下拉**。桌面筛选栏的每个控件必须说清自己筛什么：搜索框 placeholder 是“搜索任务”，tag 控件是“标签”多选 select，日期控件是“日期”条件 select / popover，状态控件是“状态”。如果某一维没有可选项，该控件隐藏或 disabled 并说明原因，不能显示一个点开后仍只有“筛选”的 option。
- `US-109b` **组合筛选必须可解释**。筛选栏要能显示当前条件摘要，例如 `tag:#work,#writing · 日期:本周 · 状态:待办`；用户能一键清空临时筛选。空结果时说明是筛选导致的，并提供“清空筛选”动作。
- `US-109c` **保存视图**：用户可以把常用 tag / date / status / search / summary 组合保存成固定视图，比如“Q1”“工作”“本周复盘”，之后一键切换。保存视图只是 UI preset，不改 markdown、不创建新数据模型；保存的是条件集合与汇总配置，不保存当时的任务列表快照。
- `US-109d` **tag 多选 select 的选项来源**。tag 选项按“当前 tab / 保存视图 / 日期 / 状态等非 tag 条件”筛出的候选集合计算，并显示每个 tag 的命中数；已选 tag 即使命中数变成 0 也要保留为 selected chip，直到用户手动移除或清空筛选。选中多个 tag 默认是 AND 语义：任务必须同时包含这些 tag 才显示。
- `US-109e` **日期筛选控件形态**。日期控件默认展示“日期”，点开后是单选列表：全部、逾期、今天、明天、本周、下周、本月、未排期、自定义日期、自定义范围。选择“自定义日期”显示一个日期选择器；选择“自定义范围”显示开始 / 结束两个日期选择器。用户看到的日期按 locale 显示，保存视图和 CLI 表达仍用稳定 token：`all / overdue / today / tomorrow / week / next-week / month / unscheduled / YYYY-MM-DD / FROM..TO`。
...
- `US-117` 筛选栏与保存视图栏要能承载长期使用：默认视图、保存视图、临时筛选三者分清。用户切换保存视图后可以继续临时改筛选；只有点“保存/更新视图”才写入设置。移动端不能把桌面完整筛选栏塞进首屏,只能保留一个“筛选”入口；这个入口打开 bottom sheet，里面仍按 US-109a 展示搜索 / 标签 / 日期 / 状态等明确控件。
...
- `US-151` 顶层 Task Card 必须在标题下方显示该任务 markdown 里写的 `#tag`。范围包含周 / 月 / 未排期 / 搜索结果 / Today 等顶层任务卡；嵌在父卡内的 subcard 为紧凑树节点,不强制显示 tag。tag 文案必须保留 markdown 原文（例如 `#work` / `#1象限`）,不翻译、不替换；同一任务重复写同一个 tag 时 UI 只显示一次。

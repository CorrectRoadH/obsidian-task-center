...
实现层面：`nestUnder` 的返回签名已经是 `{ before, after, unchanged, crossFile, undoOps: UndoOp[] }`——失败"重复"路径下返回**仅含 parent 插入那一条 op** 的 `undoOps`，由 view 入栈，toast 文案多加一行 "撤销可移除 parent 那份重复"。`applyUndoOps` 的 mtime drift 校验同样适用——只要 parent 文件在重复期间未被外部再次修改，撤销干净；被改了 → `undo_diverged` 拒绝撤销。
...
`view/tree.ts` 导出一个纯函数 `renderTaskTree(roots: ParsedTask[], all: ParsedTask[]): TreeNode[]`，TreeNode 是中性结构（`{depth, label, meta}[]`）。CLI 把它转 box-drawing 文本；GUI 把它转 DOM。**两边走同一份 traversal + 排序规则**（UX §16-6）。
...
interface UndoEntry {
  ...
  capturedMtime: Map<string, number>; // {path → mtime} 写入时的快照；undo 校验
}
```

`writer.applyUndoOps(files, ops)` 已实现：把 `op.after` 处替换回 `op.before`，写入前**逐文件 mtime 校验**——任何被影响文件 mtime 大于快照 ⇒ throw `undo_diverged`，撤销整体取消，toast：`⚠ 「<title>」自上次变更后被外部修改，撤销不安全已停止`（UX §6.7）。
...
3. **drift 检测**：写之前外部把文件 mtime 改大，断言 `applyUndoOps` 抛 `undo_diverged`。
...
| `[data-drop-zone="trash"]` | 底部垃圾桶 | 拖拽 drop target |
...
`i18n/<locale>.ts` 里的每条 `value` 都过校验：**不得包含**

- Obsidian Tasks emoji 字段标记：`⏳ 📅 🛫 ✅ ❌ ➕ 🔁 🔺 ⏫ 🔼 🔽 ⏬`
- inline field 字段名（任何 `[xxx::` 形式）
- 任何 `#` 开头的 hashtag 字符串

包含 ⇒ 编译 fail。原因：i18n 字符串混入这些字面会让翻译者改坏数据语义。
...
- [ ] 卡片树渲染走 `view/tree.ts` 共享 traversal，不在 view 里另写一份。
- [ ] 颜色仅来自 Obsidian CSS 变量，不出现 `#xxx` / `rgb(...)`。
- [ ] 拖拽 dwell 用 rAF + `performance.now()`，不用 `setTimeout(600)`。
- [ ] Undo 应用前校验目标文件 mtime 未变，drift 时 throw `undo_diverged`。

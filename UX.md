...
- 筛选栏包含明确控件，不使用一个含义泛化的“筛选”下拉：
  ...
  - 日期：按钮 + 时间范围 popover，不使用原生 select，也不要求用户手输日期。桌面两栏：左侧快捷范围（全部、逾期、今天、明天、本周、下周、本月、未排期），右侧日历视图（月切换、星期头、日期格）。日历视图固定按范围工作：先点开始日期，再点结束日期后应用闭区间；结束早于开始时自动交换。移动端堆叠为两列快捷范围 + 下方日历。快捷项是轻量 row/chip，不做满宽大主按钮；当前选中只用边框/浅底强调。
  - 状态：按钮 + 轻量 popover，不使用原生 select。选项为状态（全部）、待办、完成、放弃；点击即过滤并关闭，选中态使用同日期快捷范围一致的浅底/边框。
- 输入或选择后即过滤（文本输入 ≥ 300ms debounce；按钮 / popover 选择立即生效）。
- **筛选不改文件**，只改可见集合。
- **保存过滤视图**只是筛选 preset：默认视图、保存视图、临时筛选三者分清。切换保存视图后可以继续临时改筛选；只有点“保存过滤视图”并命名后才写入设置。移动端只露出筛选入口，完整控件在 bottom sheet 内（US-117）。

...
| Hover | 鼠标停留 | 阴影微抬 (`--shadow-s`)；右上角 fade-in `+ 子任务`（US-141）+ 三点更多按钮 |
| 选中 | 单击 | border 改成强调色 (`--interactive-accent`)；可接收键盘命令（README 卡级快捷键） |
...
- **每张卡顶层右上**显示 `+ 子任务` 按钮（hover 出现，US-141）。点击 = 在该卡末尾插入一行 `- [ ] ` 子任务，进入即时改名状态（US-161）。

### 5.4 Source edit dialog（点击卡片主路径）

点击任意 Task Card 打开源 Markdown 编辑对话框（US-168）。这个入口替代"双击打开源文件"和"hover 看上下文"作为主路径：

```
┌──────────────────────────────────────────────────────────────┐
│  Tasks/Inbox.md:L42                                  [打开源] │
│  ───────────────────────────────────────────────────────────  │
│                                                              │
│  <Obsidian 原生 Markdown 编辑器>                              │
│  ...                                                         │
│  - [ ] 父任务                                                │
...
- **主入口**：单击卡片。普通看板卡、Today 卡、保存视图过滤后的卡都走同一个 `open task source dialog` 动作。
- **定位**：打开后光标落在任务原始 markdown 行开头，编辑器把该行滚动到可视区域中间；不是只滚到附近，也不是只高亮卡片。
- **编辑能力**：对话框里必须是 Obsidian 原生 Markdown 编辑体验，用户可以直接改当前任务、加/删/改子任务、查看上下文。禁止用只读 `MarkdownRenderer` 冒充。
- **保存与刷新**：用户编辑后按 Obsidian 原生保存语义落盘；对话框关闭或文件变更事件到达后，看板刷新，卡片内容与子任务树同步。
- **关闭**：Esc / 点击遮罩 / 右上关闭都关闭对话框；关闭前不丢 Obsidian 编辑器已写入的内容。
- **辅助入口保留**：右键菜单"打开源文件"仍打开普通 Obsidian leaf；双击可以保留为同一对话框动作，但不再是用户必须知道的入口。
- **互斥**：打开 source edit dialog 前关闭 hover popover；对话框打开期间不再弹 hover popover。
- **移动端**：手机上同一动作可落为全屏/底部 sheet 形态，但仍必须是可编辑源 markdown，不是只读预览。若原生 editor 嵌入移动 sheet 不稳定，先走全屏 modal，并在验收里注明。

### 5.5 Hover popover（辅助路径）

鼠标停留卡上 ≥ 700ms 弹一个 popover（US-165）：
...
- popover 700ms 才弹，避免拖拽时误弹。
- popover 自身可悬停（不会因为离开原卡而立刻消失）；离开 popover + 卡 ≥ 200ms 才隐。
- ESC 关闭。
- 它只负责轻量上下文预览，不承担编辑；编辑主路径是 US-168 source edit dialog。
...
### 6.5 点标题改名（Inline rename）

- 单击卡片标题文字进入编辑（US-161）。
- 编辑时显示一个原生 input，宽度撑满卡片可用宽度。
- `Enter` 提交、`Esc` 回退、点编辑框外提交。
- 提交后**字节级保留**所有非标题部分（emoji / tag / inline field / block 锚点 / 优先级符号）（US-161 / US-407）。
- 改名只改"标题文本"那一段；行内其他字符的相对顺序不变。
...
| Hover popover 延时 (ms) | 700 | *设计补全*：可在 200–1500 之间调 | — |
...
- [ ] 点标题改名，emoji / tag / inline field 字节级保留。（US-161 / US-407）
- [ ] `+ 子任务` 加子任务，新子继承父的 `⏳`。（US-141 / US-144）

...
- 子任务在父卡内**递归显示所有层级**（US-142）。每深一层缩进 16px，前缀 `├` `└` 树枝符（用 box-drawing 字符，方便 CLI 和 GUI 一致）。
...
4. **多行块**（任务 + 子任务）用 box-drawing 字符 (`├ └ │`) 表达层级，与 GUI 的卡内子任务视觉一致。
...
3. **撤销栈是 UI 状态**，不是文件级。每条 undo 记录前后两版的"该行 markdown 字节"，重做时校验文件未被外部改（§6.7）。
...
6. **CLI 输出与 GUI 卡子任务用同一个 box-drawing 树渲染器**——避免两边规则漂移。

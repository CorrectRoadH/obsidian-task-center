...
```
┌──────────────────────────────────────────────────────────────────────┐
│  今日·M 周·M 月·M 已完成·M 未排期·M  🔎 [搜索任务...] [#不重要 +1] [日期] [状态] [< 今 >] + Add │
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
...
- 筛选栏包含明确控件，不使用一个含义泛化的“筛选”下拉：
  ...
  - 标签：按钮 + popover，不使用原生 `<select multiple>`。按钮默认显示“标签”，选 1 个时直接显示该 tag，选多个时显示“第一个 tag +N”；不要在按钮旁边再铺外部 tag chip。popover 顶部是搜索框，下面是自绘 checkbox 行。选项来自当前 tab / 保存视图 / 日期 / 状态等非 tag 条件下仍存在的合法 `#tag`，每项显示命中数；取消选择在 popover 内完成。
  - 日期：按钮 + 时间范围 popover，不使用原生 select，也不要求用户手输日期。桌面两栏：左侧快捷范围（全部日期、逾期、今天、明天、本周、下周、本月、未排期），右侧日历视图（月切换、星期头、日期格）。日历视图固定按范围工作：先点开始日期，再点结束日期后应用闭区间；结束早于开始时自动交换；右侧标题区提供“清空时间”。移动端堆叠为两列快捷范围 + 下方日历。工具栏按钮只显示紧凑日期摘要，不能撑破相邻按钮。快捷项是轻量 row/chip，不做满宽大主按钮；当前选中只用边框/浅底强调。
  - 状态：按钮 + 轻量 popover，不使用原生 select。按钮默认显示“状态”，选项为全部状态、待办、完成、放弃；点击即过滤并关闭，选中态使用同日期快捷范围一致的浅底/边框。
- 输入或选择后即过滤（文本输入 ≥ 300ms debounce；按钮 / popover 选择立即生效）。
- **筛选不改文件**，只改可见集合。
- **保存过滤视图**只是筛选 preset：默认视图、保存视图、临时筛选三者分清。切换保存视图后可以继续临时改筛选；只有点“保存过滤视图”并命名后才写入设置；没有任何筛选条件时保存按钮 disabled，不能保存空 preset。移动端只露出筛选入口，完整控件在 bottom sheet 内（US-117）。
...
| 保存视图 | 空 | 保存搜索 / tags / 日期 / 状态 / 汇总配置 preset；切换 preset 后仍可临时改筛选，只有“保存/更新视图”写入设置；不保存独立“分组”条件 | US-109 / US-117 |

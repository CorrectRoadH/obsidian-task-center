...
```
┌──────────────────────────────────────────────────────────────────────┐
│  今日·M 周·M 月·M 已完成·M 未排期·M  🔎 [筛选...]    [< 今 >]  + Add  │  ← 顶部 (US-105 / US-101 / US-720)
├──────────────────────────────────────────────────────────────────────┤
│                                                                      │
...
- **tag 不参与内置分组**：`#1象限`、`#now` 等都只是普通 tag。用户要按 tag 看集合时，用筛选栏 tag 多选 select；常用组合保存成视图（US-109 / US-301）。
...
│  ────────────────────────────────────────────────────────────  │
│  [Today]  [Tomorrow]  [周六]  [下周]  [#work]  [#writing]          │  ← 可点 quick-chip 行（点 = prefill 到 input）
...
- 筛选栏包含明确控件，不使用一个含义泛化的“筛选”下拉：
  ...
  - 标签：多选 select，选项来自当前 tab / 保存视图 / 日期 / 状态等非 tag 条件下仍存在的 `#tag`，每项显示命中数；已选 tag 以 chip 展示。
  - 日期：单选条件 select / popover，不是裸输入框。一级选项为：全部、逾期、今天、明天、本周、下周、本月、未排期、自定义日期、自定义范围；自定义项再展开 date picker。
  - 状态：待办 / 已完成 / 已放弃 / 全部。
- 输入或选择后即过滤（文本输入 ≥ 300ms debounce；select 变更立即生效）。
- **筛选不改文件**，只改可见集合。
- **保存视图**只是筛选 preset：默认视图、保存视图、临时筛选三者分清。切换保存视图后可以继续临时改筛选；只有点“保存/更新视图”才写入设置。移动端只露出筛选入口，完整控件在 bottom sheet 内（US-117）。
...
| 保存视图 | 空 | 保存搜索 / tag / 日期 / 状态 / 汇总配置 preset；切换 preset 后仍可临时改筛选，只有“保存/更新视图”写入设置 | US-109 / US-117 |
| AI skill 安装指引 | 展示可复制命令 | 只在设置页作为帮助入口，移动端不暴露不可用 CLI 能力 | US-215 |

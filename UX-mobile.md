...
| Surface | 移动端形态 | vs 桌面 |
| --- | --- | --- |
| **Task Center 主视图** | 全屏 leaf；底部 sticky 操作条（放弃目标区 + Quick Add 按钮） | 桌面是 tab 内一个 leaf；移动端通常占满整个屏幕 |
...
> `US-501`：移动端不支持的桌面功能（CLI、键盘快捷键等）静默 no-op，不弹错误 / 警告；hover popover 不再作为任何端的功能存在。
...
│       [卡] [卡] [卡] ...               │
├────────────────────────────────────────┤
│   ⏹ 放弃          ➕ Add               │  ← Bottom action bar (sticky)
└────────────────────────────────────────┘
```
...
- **Bottom action bar**：放弃目标区（sticky，拖入 = 放弃，US-123）+ `➕ Add` 按钮（唤起 Quick Add bottom sheet）。两侧可达；可见 UI 不使用垃圾桶 / 删除图标。
...
> `US-505`：移动端卡片紧凑模式——meta 行合并 / 子任务默认显 1 层 / 拖动加 `scale 1.04`；未排期区域与卡片宽度占满可用宽度。
...
按住卡片 ≥ 500ms 弹半屏 bottom sheet，内容 = 桌面右键菜单（UX.md §5.5）的移动版本。源文件查看/编辑不在长按菜单里重复出现，统一由单击卡片打开 US-168 源 Markdown 编辑层：
...
> `US-506`：长按卡片 ≥ 500ms（且不移动）弹移动端操作 sheet。如果按住期间移动 ≥ 4px（最早 250ms 后生效），改走拖拽，long-press menu 取消；两者**互斥不并发**。
...
- **放弃目标区**：始终在 bottom action bar 内 sticky 可见。
- **嵌套到另一卡**：松手在另一卡卡体上 = 跨文件 nest（US-125），等价桌面拖到另一卡，toast + undo（UX.md §6.2）。

> `US-507`：移动端拖拽 long-press 250ms 进入 drag mode；跨 tab dwell 800ms；自动滚动边缘 60px；只改变启动方式和布局，不改变 US-121 / US-122 / US-125 / US-123 / US-126 的落点语义。

### 5.3 滑动手势（移动端独有）

- **左滑卡片** → 标完成（等价 `Space` 切完成）。手势确认窗口：露出绿底"完成"提示 + 1 秒撤销（toast + undo）。
- **右滑卡片** → 放弃（等价拖到放弃目标区）。手势确认窗口：露出红底"放弃"提示 + 1 秒撤销。
...
- [ ] 顶部 tab 横向滚动；底部 sticky `放弃 + ➕` 按钮单手大拇指可达，触控高度 ≥ 44px
...
- [ ] 长按 ≥ 500ms 弹动作菜单；源编辑只由单击卡片进入（US-506 / US-168）
